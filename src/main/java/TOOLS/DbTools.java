...
public class DbTools {
    ...
    public static void databaseCreation() {
        try {
            ...
            String imagetable = "create table imagetable(imageid varchar(6) primary key,image blob not null,imagedate date not null)";
    ...
    public static boolean imagetableCreation() {
        try {
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost/GYM2", "yogesh", "java");
            String query = ("insert into imagetable values(?,?,?)");
            PreparedStatement ps = con.prepareStatement(query);
            ps.setString(1, "49h");
            ps.setString(2, "Null");
            ps.setString(3, "1999-05-18");
            ps.executeUpdate();
        } catch (Exception e) {
            System.out.println(e);
